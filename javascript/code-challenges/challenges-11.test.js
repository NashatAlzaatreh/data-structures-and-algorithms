...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let filtered = input.map((elm) =>
    elm.filter((num) => typeof num === "number" && num % 5 === 0)
  );
  return filtered.map((elm) => elm.map((num) => Math.pow(2, num)));
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data
    .filter((key) => key.gender === "male" || key.gender === "female")
    .reduce((acc, elm, i) => {
      const len = data.filter(
        (key) => key.gender === "male" || key.gender === "female"
      ).length;
      if (i < len - 1) {
        return (acc = acc + elm.name + " and ");
      } else {
        return (acc += elm.name);
      }
    }, "");
